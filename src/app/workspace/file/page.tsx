  ...
  return (
    <div className="p-6 max-w-[1500px] mx-auto font-sans aesthetic flex flex-col h-screen bg-gray-50/50">
      <div className="flex justify-between items-center border-b border-gray-300 pb-4 mb-6 shrink-0">
        <h1 className="text-3xl font-extrabold uppercase text-gray-900">File Import</h1>
      ...
      {(previewTable.length > 0 || previewImage) && (
        <div className="fixed inset-0 bg-gray-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-8">
          <div className="bg-white rounded-2xl w-full max-w-6xl max-h-[80vh] flex flex-col overflow-hidden shadow-2xl border border-gray-300">
            <div className="p-6 border-b border-gray-200 flex justify-between items-center bg-gray-50">
              <div>
                <h2 className="text-xl font-extrabold uppercase tracking-widest text-gray-900">Source Analysis Preview</h2>
              ...
              <div className="flex gap-4">
                <button onClick={() => {setPreviewTable([]); setPreviewImage(null); setPreviewFile(null);}} disabled={isProcessingFile} className="px-6 py-3 bg-white border border-gray-300 rounded-xl font-bold uppercase disabled:opacity-50 text-gray-900 shadow-sm">Cancel</button>
                <button onClick={executeBulkEnrichment} disabled={isProcessingFile} className="px-8 py-3 bg-gray-900 text-white rounded-xl font-extrabold uppercase tracking-wider disabled:opacity-50 shadow-sm">
            ...
            <div className="p-6 overflow-auto flex-1 flex flex-col items-center justify-start">
              {previewImage ? (
                 <img src={previewImage} alt="Upload Preview" className="max-h-full object-contain rounded-lg border border-gray-300 shadow-sm" />
              ) : (
                <>
                  <table className="w-full text-left text-sm whitespace-nowrap bg-white border border-gray-200 rounded-lg">
                    <thead className="bg-gray-100 uppercase font-bold text-xs text-gray-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 flex-1 min-h-0 pb-4">
        ...
        <div className="md:col-span-5 border border-gray-300 p-6 rounded-2xl bg-white flex flex-col shadow-sm min-h-0 h-full">
          <h2 className="font-extrabold uppercase mb-6 tracking-wider shrink-0 text-gray-900">2. Import Target Source</h2>
          ...
          <div className="space-y-4 shrink-0">
            ...
            <button onClick={() => fileInputRef.current?.click()} disabled={!selectedFolderId} className="w-full bg-gray-50 text-gray-900 px-4 py-6 rounded-xl font-bold uppercase tracking-wider disabled:opacity-50 border border-dashed border-gray-400 hover:border-gray-900 transition-colors hover:bg-gray-100">
              Select CSV / Excel / Image
            </button>
          </div>
